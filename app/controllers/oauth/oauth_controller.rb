...
class Oauth::OauthController < Devise::OmniauthCallbacksController
  def dhis2_login
    ...
    if oauth_client_id.blank?
      flash[:failure] =
"Log-in failed: program with ID #{params['program_id']} is not configured for sign-in with DHIS2"
      redirect_to("/users/sign_in")
      return
    end
    ...
    redirect_to(url_redirect, allow_other_host: true)
  end
  ...
  def callback
    ...
    user_info = RestClient.get(program.project_anchor.project.dhis2_url + "/api/me",
    ...
    if user
      ...
      flash[:failure] =
"Log-in failed: no ORBF2 user associated to the DHIS2 user with name #{user_info['displayName']} and ID #{dhis2_user_ref}"
      redirect_to("/users/sign_in")
      nil
    end
  end
