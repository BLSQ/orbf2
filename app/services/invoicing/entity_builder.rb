module Invoicing
  class EntityBuilder
    ...
    def to_facts(org_unit)
      ...
      group_set_facts = pyramid.org_unit_groups(org_unit.organisation_unit_groups.map { |n| n["id"] }).map do |group|
        next if group.group_set_ids.empty?
      ...
      facts = parent_ids.each_with_index
                        ...
                        .to_h
      facts.merge(group_set_facts.compact.flatten(1).to_h)
    end
  end
end
