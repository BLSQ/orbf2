...
(function($, window, document) {
  ...
  SearchableOptionList.prototype = {
    ...
    defaults: {
      ...
      events: {
        ...
        onScroll: function() {
          var posY =
              Math.floor(this.$input.offset().top) -
              Math.floor(this.config.scrollTarget.scrollTop()) +
              Math.floor(this.$input.outerHeight()),
    ...
    init: function() {
      ...
      this.config.multiple =
        this.config.multiple || this.$originalElement.attr("multiple");
      ...
      this.$showSelectionContainer
        .detach()
        .appendTo("#" + this.config.selectionDestination);
    ...
    _getNameAttribute: function() {
      return (
        this.config.name ||
        this.$originalElement.data("sol-name") ||
        this.$originalElement.attr("name")
      );
    },
    ...
    _registerWindowEventsIfNeccessary: function() {
      if (!window[this.WINDOW_EVENTS_KEY]) {
        $(document).click(function(event) {
          ...
          var $closestSolContainer = $(event.target).closest(
              ".sol-inner-container"
            ),
            $clickedWithinThisSolContainer = $closestSolContainer
              .first()
              .parent(".sol-container");
    ...
    _initializeUiElements: function() {
      ...
      this.$input = $('<input type="text"/>').attr(
        "placeholder",
        this.config.texts.searchplaceholder
      );

      this.$noResultsItem = $('<div class="sol-no-results"/>')
        .html(this.config.texts.noItemsAvailable)
        .hide();

      this.$caret = $(
        '<div class="sol-caret-container"><b class="caret"/></div>'
      ).click(function() {
        self.toggle();
      });
      var $inputContainer = $('<div class="sol-input-container"/>').append(
        this.$input
      );
      ...
      if (this.config.multiple) {
        ...
        if (this.config.showSelectAll) {
          var $deselectAllButton = $('<a href="#" class="sol-deselect-all"/>')
      ...
      if ($form && $form.length === 1 && !$form.data(this.WINDOW_EVENTS_KEY)) {
        var resetFunction = function() {
          ...
          if (
            ...
            $.isFunction(self.config.events.onChange)
          ) {
        ...
        .on("keydown", function(e) {
          ...
          if (!self.$noResultsItem.is(":visible")) {
            ...
            if (keyCode === 40 || keyCode === 38) {
              ...
              $nextHighlightedOption = $(
                $allVisibleOptions[indexOfNextHighlightedOption]
              ).addClass("keyboard-selection");

              self.$selection.scrollTop(
                self.$selection.scrollTop() +
              ...
              $currentHighlightedOption = self.$selection.find(
                ".sol-option.keyboard-selection input"
              );
    ...
    _applySearchTermFilter: function() {
      ...
      this.$selectionContainer
        .find(".sol-filtered-search")
        .removeClass("sol-filtered-search");
    ...
    _findTerms: function(dataArray, searchTerm) {
      ...
      $.each(dataArray, function(index, item) {
        if (item.type === "option") {
          var $element = item.displayElement,
            elementSearchableTerms = (item.label + " " + item.tooltip)
              .trim()
              .toLowerCase();
          ...
          var amountOfUnfilteredChildren = item.displayElement.find(
            ".sol-option:not(.sol-filtered-search)"
          );
      ...
      this._setNoResultsItemVisible(
        this.$selectionContainer.find(".sol-option:not(.sol-filtered-search)")
          .length === 0
      );
    },
      ...
      if (this.$originalElement.prop("tagName").toLowerCase() === "select") {
        ...
        $.each(this.$originalElement.children(), function(index, item) {
          ...
          if (itemTagName === "option") {
            ...
            self._showErrorLabel(
              "Invalid element found in select: " +
                itemTagName +
    ...
    _processDataItems: function(solItems) {
      if (!solItems) {
        this._showErrorLabel(
          "Data items not present. Maybe the converter did not return any values"
        );
        return;
      }
      ...
      $.each(solItems, function(index, item) {
        if (
          item.type === SearchableOptionList.prototype.SOL_OPTION_FORMAT.type
        ) {
          self._renderOption(item);
        } else if (
          item.type ===
          SearchableOptionList.prototype.SOL_OPTIONGROUP_FORMAT.type
        ) {
    ...
    _renderOption: function(solOption, $optionalTargetContainer) {
      var self = this,
        ...
        $inputElement,
        $labelText = $('<div class="sol-label-text"/>').html(
          solOption.label.trim().length === 0 ? "&nbsp;" : solOption.label
        ),
        $label,
        $displayElement,
    ...
    _selectionChange: function($changeItem, skipCallback) {
      ...
      if (
        this.$originalElement &&
        this.$originalElement.prop("tagName").toLowerCase() === "select"
      ) {
        var self = this;
        this.$originalElement.find("option").each(function(index, item) {
          ...
          if ($currentOriginalOption.val() === $changeItem.val()) {
            $currentOriginalOption.prop(
              "selected",
              $changeItem.prop("checked")
            );
    ...
    _addSelectionDisplayItem: function($changedItem) {
      ...
      if (!$existingDisplayItem) {
        ...
        if (
          (this.config.multiple || this.config.allowNullSelection) &&
          !$changedItem.prop("disabled")
        ) {
    ...
    open: function() {
      if (this.isClosed()) {
        ...
        this.config.scrollTarget
          .bind("scroll", this.internalScrollWrapper)
          .trigger("scroll");
  ...
  $.fn.searchableOptionList = function(options) {
    ...
    this.each(function() {
      var $this = $(this),
        $alreadyInitializedSol = $this.data(
          SearchableOptionList.prototype.DATA_KEY
        );
