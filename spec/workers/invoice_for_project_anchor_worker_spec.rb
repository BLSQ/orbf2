...
RSpec.describe InvoiceForProjectAnchorWorker do
  ...
  describe "new engine" do
    ...
    let(:dataset_ext_ids) { %w[ds-0 ds-1 ds-2] }
  ...
  describe "retry logic" do
    it "doesn't fail if equation fails" do
      expect(InvoicingJob).to receive(:execute) { raise Hesabu::Error, "In equation and so on" }
      expect {
        worker.perform(project.project_anchor.id, 2015, 1, ["Rp268JB6Ne4"])
      }.to_not raise_error
    end
    ...
    it "fails if hesabu error other than equation fails" do
      expect(InvoicingJob).to receive(:execute) { raise Hesabu::Error, "Some error in Hesabu" }
  ...
  def sorted_datavalues(json)
    json["dataValues"].sort_by { |e| [e["dataElement"], e["orgUnit"], e["period"]] }
