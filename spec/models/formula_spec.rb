...
RSpec.describe Formula, type: :model do
  def new_formula(args)
    rule = Rule.new
    Formula.new(args.merge(description: "description", rule: rule))
  end
  ...
  describe "Expression validation" do
    it "should accept activity expression" do
      formula = new_formula(code: "sample_expression", expression: "variable - 456")
      expect(formula.valid?).to be true
    end

    it "should reject formula activity expression bad if" do
      formula = new_formula(code: "sample_expression", expression: "IF (bad_if_statement - 456")
      formula.valid?
    ...
    it "should reject formula activity expression unknown function" do
      formula =new_formula(code: "sample_expression", expression: "unknown(bad_if_statement - 456)")
      formula.valid?
      expect(formula.errors[:expression]).to eq(["Undefined function unknown"])
    end

    it "should validate formula for SUM expression" do
      formula =new_formula(code: "sample_expression", expression: "SUM(%{amount_values})")
    ...
    it "should reject dangerous expression" do
      formula = new_formula(code: "sample_expression", expression: "`ls -als`")
